...
int fDriver::read(char* list[]) {
    ...
    /* -- Start lexo timing -- */    
    high_resolution_clock::time_point startLexo = high_resolution_clock::now();
...
//         if (fIn.fail()){
//             cerr << "Failed to open file: " << list[i] <<endl;
...
//         }
    ...
    cout << stemTime.count() << endl;
    /* -- Stop lexo timing -- */
    high_resolution_clock::time_point stopLexo = high_resolution_clock::now();
    duration<double> durationLexo = duration_cast<duration<double>>(stopLexo - startLexo);
    cout << durationLexo.count() << endl;
...
void fDriver::calcMatrix() {
    high_resolution_clock::time_point startCalcMatrix = high_resolution_clock::now();
...
//     }
    /* -- Stop lexo timing -- */
    high_resolution_clock::time_point stopCalcMatrix = high_resolution_clock::now();
    duration<double> durationCalcMatrix = duration_cast<duration<double>>(stopCalcMatrix - startCalcMatrix);
    cout << durationCalcMatrix.count() << endl;
}
    ...
    for(itA = A.freqMap.begin(); itA != A.freqMap.end(); ++itA) {
        ...
        for(itB = B.freqMap.begin(); itB != B.freqMap.end(); ++itB) {
            ...
            if(currA == currB) {
                ...
                numFOP += 3;
                numByte += 3; // load i,j,sum, write to sum;
                sum += i * j; // get the TFIDF for both words in the docs
...
#pragma omp parallel num_threads(2)
    {
        ...
        numByte += 2; // load i,j
        ++numFOP;
        return i * j; // perform the TFIDF calculation by calling TF & IDF
}
...
double fDriver::calcIDF(const string &sIn) {
    ...
    for(vector<lexo>::iterator itV = docs.begin(); itV < docs.end(); ++itV) {
        ...
        ++numByte; // load value of results map
