...
//TODO needs i guess should be: need not ne
/* ----STEP METHODS 1-8---- */
string Stem::Step1(string sIn) {
    //TESTING
    if(sIn == "dune" || sIn == "dunes")
//         cout << "x" << endl;
...
string Stem::Step3(string &sIn) {
    if(Find(sIn,"eed") || Find(sIn,"eedly")) { // row 1
        if(Find(Region1(sIn),"eed") || Find(Region1(sIn),"eedly")) // row 1 conditions
...
const string Stem::step3Replace(string &sIn, const string &suffix) {
    ...
    if((back1 == 'a' && back == 't') || (back1 == 'b' && back == 'l') || (back1 == 'i' && back == 'z')) // if preceder ends in at, bl, or iz
        ...
        lSuffix = 1; // removing only the last char
...
const bool Stem::isShortSyllable(const string &sIn) {
    ...
    if(sIn.length() <= 2 
        ...
        return true;
    // if its smaller then two then it has to be false
    if(sIn.length() <= 2)
        return false;
